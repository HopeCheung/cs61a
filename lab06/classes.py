...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            return
        for char in self.place.characters:
            if person == char:
                print(self.place.characters[char].name, 'says:', self.place.characters[char].message)
                return
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            return
        for item in self.place.things:
            if thing == item:
                print('Player takes the', self.place.things[item].name)
                self.backpack = self.backpack + [self.place.things[item]]
                del self.place.things[item]
                return 
